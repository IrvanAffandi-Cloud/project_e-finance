...
export default function KategoriPage() {
  ...
  // UBAH: actions jadi flex-row, ukuran tombol pakai flex-1 biar sejajar 1 baris
  const baseSwalClass = {
    ...
    actions: 'w-full flex flex-row gap-2 mt-5',
    ...
    cancelButton: 'flex-1 h-10 flex items-center justify-center bg-[#F5F5F7] border border-transparent text-gray-500 font-black text-[10px] tracking-[0.15em] uppercase px-2 rounded-xl hover:bg-gray-200 active:scale-95 transition-all duration-200'
  }
  ...
  const handleTambah = async () => {
    const { value: form } = await Swal.fire({
      ...
      `,
      showCancelButton: true, confirmButtonText: 'SIMPAN', cancelButtonText: 'BATAL', buttonsStyling: false, customClass: baseSwalClass,
      preConfirm: () => {
  ...
  const handleAksi = async (k: any) => {
    ...
    if (aksi.isConfirmed) {
      ...
      const hapus = await Swal.fire({ title: 'YAKIN HAPUS?', text: 'Riwayat transaksi akan menjadi "TANPA KATEGORI".', icon: 'warning', showCancelButton: true, confirmButtonText: 'HAPUS', cancelButtonText: 'BATAL', customClass: { ...baseSwalClass, confirmButton: baseSwalClass.confirmButton.replace('bg-[#0B214A]', 'bg-red-600') }, buttonsStyling: false })
    ...
    <main className="min-h-screen bg-[#FBFBFD] text-[#1D1D1F] flex flex-col items-center font-sans pb-24">
      ...
      <header className="sticky top-0 z-50 w-full h-[68px] rounded-b-[2.5rem] bg-gradient-to-br from-blue-700 via-blue-600 to-blue-800 shadow-[0_10px_30px_rgba(37,99,235,0.3)] border-b border-blue-400/40 overflow-hidden flex flex-col items-center justify-center pt-1">
        ...
        <div className="w-full max-w-xl relative flex flex-col items-center justify-center">
          ...
          {/* UBAH: Tag H2 dengan styling identik seperti text Dashboard di depan */}
          <h2 className="text-white font-black text-[17px] tracking-[0.4em] uppercase text-center drop-shadow-md z-10 leading-none mt-1">
            KATEGORI
          </h2>
        </div>
      </header>
