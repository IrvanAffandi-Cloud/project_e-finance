    ...
    <main className="min-h-screen bg-[#FBFBFD] text-[#1D1D1F] flex flex-col font-sans pb-20 selection:bg-red-200 overflow-x-hidden items-center">
      ...
      <header className="sticky top-0 z-50 w-full h-[68px] rounded-b-[2.5rem] bg-gradient-to-br from-orange-600 via-red-500 to-red-600 shadow-[0_10px_30px_rgba(239,68,68,0.3)] border-b border-red-300/40 overflow-hidden flex flex-col items-center justify-center pt-1">
        ...
        <div className="w-full max-w-xl relative flex flex-col items-center justify-center">
          ...
          <h1 className="text-white font-black text-[17px] tracking-[0.4em] uppercase drop-shadow-md z-10 leading-none">PENGELUARAN</h1>
        </div>
      </header>
      ...
      <div className="w-full max-w-xl px-4 mt-6 flex flex-col gap-4">
        ...
        <div className="bg-white p-4 rounded-[1.5rem] shadow-sm border border-gray-200 relative overflow-hidden">
          ...
          <form onSubmit={handleSimpan} className="flex flex-col gap-2">
            <input name="nominal" type="text" inputMode="numeric" onInput={(e) => { e.currentTarget.value = e.currentTarget.value.replace(/[^0-9]/g, '').replace(/\B(?=(\d{3})+(?!\d))/g, '.') }} placeholder="Jumlah" className="w-full h-10 px-3 bg-[#F5F5F7] border border-transparent rounded-xl text-center font-bold text-[12px] outline-none focus:border-red-400 focus:bg-white tracking-widest transition-all placeholder:text-gray-400" />
            
            <select name="kategori" defaultValue="" className="w-full h-10 px-3 bg-[#F5F5F7] border border-transparent rounded-xl text-center font-bold text-[10px] outline-none focus:border-red-400 focus:bg-white text-gray-600 uppercase tracking-widest transition-all">
            ...
            <input name="catatan" type="text" placeholder="Deskripsi" className="w-full h-10 px-3 bg-[#F5F5F7] border border-transparent rounded-xl text-center font-bold text-[10px] uppercase outline-none focus:border-red-400 focus:bg-white transition-all placeholder:text-gray-400 tracking-widest" />
            
            <button type="submit" className="w-full h-10 bg-orange-600 text-white font-bold text-[10px] uppercase rounded-xl shadow-sm hover:bg-orange-700 active:scale-95 transition-all mt-1 tracking-[0.15em]">
              SIMPAN
            </button>
          </form>
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-[1.2rem] shadow-sm overflow-hidden flex flex-col">
            {transaksi.length === 0 ? <p className="text-[9px] font-black tracking-[0.2em] text-gray-400 text-center py-6">BELUM ADA DATA</p> : 
              transaksi.map((t, i) => {
                ...
                let displayName = t.kategori?.nama_kategori || 'TANPA KATEGORI'
                ...
                return (
                  ...
                  <div key={t.id} onClick={() => !isSystemLocked && handleAksi(t)} className={`flex justify-between items-center px-4 py-2.5 border-b border-gray-100 last:border-b-0 transition-colors ${isSystemLocked ? 'bg-gray-50/50 opacity-80' : 'cursor-pointer hover:bg-red-50 active:bg-red-100'}`}>
                    <div className="flex flex-col">
                      <div className="flex items-center gap-2">
                        {/* FONT BOLD BUKAN BLACK */}
                        <span className="font-bold text-[12px] text-[#1D1D1F] uppercase tracking-tight">{displayName}</span>
                        {isSystemLocked && <span className="text-[6px] font-bold bg-red-100 text-red-600 px-1.5 py-0.5 rounded border border-red-200 uppercase tracking-widest">AUTO</span>}
                      </div>
                      <span className="text-[8px] font-bold text-gray-400 uppercase tracking-[0.1em] mt-0.5">{t.catatan || '-'}</span>
                    </div>
                    <div className="flex flex-col items-end">
                      <span className="font-bold text-[12px] text-red-600 tracking-tight">{formatRupiah(t.nominal)}</span>
