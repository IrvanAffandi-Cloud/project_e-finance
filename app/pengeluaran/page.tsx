...
export default function PengeluaranPage() {
  ...
  const loadData = async () => {
    ...
    // LIMITASI UI: Potong paksa, cuma render 50 baris teratas
  ...
  const handleSimpan = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (kategoriTerpilih?.nama_kategori === 'CICILAN BANK' || kategoriTerpilih?.nama_kategori === 'BAYAR UTANG PRIBADI') {
      return Swal.fire('DITOLAK!', 'Kategori ini khusus digunakan oleh Mesin Otomatis!', 'error')
    }
  ...
  const handleAksi = async (t: any) => {
    if (t.kategori?.nama_kategori === 'CICILAN BANK' || t.kategori?.nama_kategori === 'BAYAR UTANG PRIBADI') {
      return Swal.fire('TERKUNCI', 'Data ini dikontrol otomatis oleh sistem Cicilan/Utang. Tidak bisa diedit manual!', 'error')
    }
    ...
    const aksi = await Swal.fire({
      ...
      customClass: { popup: '!rounded-[1.5rem] !p-5', confirmButton: '!bg-orange-500 !text-white !font-bold !rounded-xl !px-5 !py-3 w-full mb-2', denyButton: '!bg-red-600 !text-white !font-bold !rounded-xl !px-5 !py-3 w-full mb-2', cancelButton: '!bg-gray-100 !text-gray-500 !font-bold !rounded-xl !px-5 !py-3 w-full', actions: '!flex !flex-col w-full' }, buttonsStyling: false
    })
    ...
    if (aksi.isConfirmed) {
      ...
      const { value: formEdit } = await Swal.fire({
        ...
        html: `<input id="swal-edit-nom" type="text" inputmode="numeric" value="${new Intl.NumberFormat('id-ID').format(t.nominal)}" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/\\B(?=(\\d{3})+(?!\\d))/g, '.')" class="w-full h-12 px-4 mb-3 bg-[#F5F5F7] border border-gray-200 rounded-xl text-center font-bold outline-none focus:border-red-500"><select id="swal-edit-kat" class="w-full h-12 px-4 mb-3 bg-[#F5F5F7] border border-gray-200 rounded-xl text-center font-bold outline-none text-gray-500 uppercase">${optionsHtml}</select><input id="swal-edit-cat" type="text" value="${t.catatan || ''}" class="w-full h-12 px-4 bg-[#F5F5F7] border border-gray-200 rounded-xl text-center font-bold text-xs uppercase outline-none focus:border-red-500">`,
        showCancelButton: true, confirmButtonText: 'SIMPAN',
        customClass: { confirmButton: '!bg-orange-500 !text-white !font-bold !w-full !rounded-xl !py-3 !mt-3', cancelButton: '!bg-gray-100 !text-gray-500 !font-bold !w-full !rounded-xl !py-3 !mt-3', popup: '!rounded-[1.5rem]' }, buttonsStyling: false,
        preConfirm: () => {
  ...
  return (
    <main className="min-h-screen bg-[#FBFBFD] text-[#1D1D1F] flex flex-col items-center px-5 pt-6 pb-20">
      <header className="w-full max-w-md flex justify-between items-center mb-6">
        <h1 className="font-bold text-[16px] uppercase tracking-wider text-red-600">PENGELUARAN</h1>
      ...
      <div className="w-full max-w-md bg-white p-6 rounded-[1.5rem] shadow-sm border border-gray-100 mb-6">
        <form onSubmit={handleSimpan} className="flex flex-col gap-4">
          <input name="nominal" type="text" inputMode="numeric" onInput={(e) => { e.currentTarget.value = e.currentTarget.value.replace(/[^0-9]/g, '').replace(/\B(?=(\d{3})+(?!\d))/g, '.') }} placeholder="NOMINAL" className="w-full h-14 px-4 bg-[#F5F5F7] border border-gray-200 rounded-xl text-center font-bold text-lg outline-none focus:border-red-500 tracking-wider" />
          ...
          <input name="catatan" type="text" placeholder="CATATAN (OPSIONAL)" className="w-full h-14 px-4 bg-[#F5F5F7] border border-gray-200 rounded-xl text-center font-bold text-xs uppercase outline-none focus:border-red-500" />
          <button type="submit" className="w-full h-14 bg-gradient-to-br from-orange-500 to-orange-400 text-white font-bold text-[11px] uppercase rounded-xl shadow-md hover:shadow-lg active:scale-95 transition-all mt-2 tracking-widest">
            SIMPAN DATA
          </button>
        </form>
      </div>
      ...
      <div className="w-full max-w-md flex flex-col gap-3">
        ...
        {transaksi.length === 0 ? <p className="text-[10px] font-bold text-gray-400 text-center py-4">BELUM ADA DATA</p> : 
          transaksi.map(t => {
            ...
            return (
              <div key={t.id} onClick={() => !isSystemLocked && handleAksi(t)} className={`bg-white p-4 rounded-[1.2rem] border border-gray-100 shadow-sm flex justify-between items-center transition-all ${isSystemLocked ? 'opacity-70 bg-gray-50/50' : 'cursor-pointer active:scale-95 hover:border-red-200'}`}>
                <div className="flex flex-col">
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-[12px] text-[#1D1D1F] uppercase">{t.kategori?.nama_kategori}</span>
                    {isSystemLocked && <span className="text-[7px] font-bold bg-red-100 text-red-600 px-1.5 py-0.5 rounded border border-red-200 uppercase tracking-widest">AUTO</span>}
                  </div>
                  <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wider mt-0.5">{t.catatan || '-'}</span>
                </div>
                <div className="flex flex-col items-end">
                  <span className="font-bold text-[13px] text-red-600">{formatRupiah(t.nominal)}</span>
