  ...
  const handleTambah = async () => {
    const htmlForm = `
      <div class="flex flex-col gap-2 text-left">
        ...
        <input id="swal-pokok" type="text" inputmode="numeric" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/\\B(?=(\\d{3})+(?!\\d))/g, '.')" placeholder="JUMLAH UTANG (RP)" class="w-full h-10 px-4 bg-[#F5F5F7] border border-transparent focus:border-blue-400 focus:bg-white rounded-xl text-center font-bold text-[12px] outline-none tracking-widest">
  ...
  const handleAksi = async (u: any) => {
    ...
    if (aksi.isConfirmed) {
      const { value: nominal } = await Swal.fire({
        ...
        showCancelButton: true, confirmButtonText: 'BAYAR SEKARANG', customClass: { ...baseSwalClass, confirmButton: baseSwalClass.confirmButton.replace('bg-[#0B214A]', 'bg-green-600').replace('hover:bg-blue-900', 'hover:bg-green-700') }, buttonsStyling: false,
        preConfirm: () => {
    ...
    <main className="min-h-screen bg-[#FBFBFD] text-[#1D1D1F] flex flex-col font-sans pb-24 items-center">
      ...
      <header className="sticky top-0 z-50 w-full h-[68px] rounded-b-[2.5rem] bg-gradient-to-br from-purple-800 via-purple-700 to-purple-900 shadow-[0_10px_30px_rgba(147,51,234,0.3)] border-b border-purple-400/40 overflow-hidden flex flex-col items-center justify-center pt-1">
        ...
        <div className="w-full max-w-xl relative flex flex-col items-center justify-center">
          ...
          <h1 className="text-white font-black text-[17px] tracking-[0.4em] uppercase drop-shadow-md z-10 leading-none">UTANG PERORANGAN</h1>
        </div>
      </header>
      ...
      <div className="w-full max-w-xl px-4 mt-6 flex flex-col gap-4">
        ...
        <div className="bg-white border border-gray-200 p-4 rounded-[1.2rem] shadow-sm flex justify-between items-center">
          <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em]">TOTAL UTANG</p>
          {/* UKURAN ANGKA PROPORSIONAL */}
        ...
        <div className="bg-white border border-gray-200 rounded-[1.2rem] shadow-sm overflow-hidden flex flex-col">
          {utang.length === 0 ? (
            <p className="text-[10px] font-bold tracking-[0.2em] text-gray-400 text-center py-6 uppercase">TIDAK ADA UTANG AKTIF</p>
          ) : (
            utang.map((u) => (
              ...
              >
                ...
                {/* WARNA HITAM SOLID (TIDAK ADA WARNA MERAH) */}
