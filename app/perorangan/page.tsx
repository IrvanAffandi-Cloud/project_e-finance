...
export default function PeroranganPage() {
  ...
  const baseSwalClass = {
    ...
    confirmButton: 'flex-1 h-10 flex items-center justify-center bg-[#0B214A] text-white font-black text-[10px] tracking-[0.15em] uppercase px-2 rounded-xl shadow-sm hover:bg-blue-900 active:scale-95 transition-all duration-200',
    cancelButton: 'flex-1 h-10 flex items-center justify-center bg-[#F5F5F7] text-gray-500 font-black text-[10px] tracking-[0.15em] uppercase px-2 rounded-xl hover:bg-gray-200 active:scale-95 transition-all duration-200'
  }
  ...
  const handleTambah = async () => {
    ...
    const htmlForm = `
      <div class="flex flex-col gap-2 text-left">
        ...
        <input id="swal-pokok" type="text" inputmode="numeric" oninput="this.value = this.value.replace(/[^0-9]/g, '').replace(/\\B(?=(\\d{3})+(?!\\d))/g, '.')" placeholder="JUMLAH" class="w-full h-10 px-4 bg-[#F5F5F7] border border-transparent focus:border-blue-400 focus:bg-white rounded-xl text-center font-bold text-[12px] outline-none tracking-widest">
