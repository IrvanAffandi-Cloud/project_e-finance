...
export default function PemasukanPage() {
  ...
  useEffect(() => {
    getKategori().then(res => {
      setKategoriList((res.kategori || []).filter((k: any) => k.tipe === 'PEMASUKAN'))
      setLoading(false)
    })
  }, [])
  ...
  if (loading) return <div className="min-h-screen bg-[#FBFBFD] flex items-center justify-center"><div className="w-8 h-8 border-[3px] border-green-500 border-t-transparent rounded-full animate-spin"></div></div>
